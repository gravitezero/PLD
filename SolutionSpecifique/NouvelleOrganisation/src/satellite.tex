...
Une fois le chantier fini, le matériel sera transporté vers le nouveau chantier si aucune maintenance préventive n'est prévu sans repasser par le siège ou un site secondaire.
        ...
        Après avoir défini l'ensemble de nos thèmes de progrès, il est apparu clairement un besoin de mettre à jour les modèles de processus existants. Nous avons mis à jour les principaux processus.\\
                ...
                \subsubsection{Processus Achat}
                    %Ci-dessous le modèle conceptuel de traitement (MCT). --> approvisionnementdepr.pdf

                    %Ci-dessous le modèle organisationnel de traitement (MOT). --> MOTapprovisionnementdepr.pdf

                \subsubsection{Processus Planification}
                    %Ci-dessous le modèle conceptuel de traitement (MCT). --> planification.pdf
                    
                    %Ci-dessous le modèle organisationnel de traitement (MOT). --> MOTplanification.pdf
                    \includegraphics[0.9\textwidth]{MOTplanification.pdf}
                
                \subsubsection{Processus Affectation/Restitution de matériels}
                    %Ci-dessous le modèle conceptuel de traitement (MCT). --> affectationmateriel.pdf

                    %Ci-dessous le modèle organisationnel de traitement (MOT). --> MOTaffectationmateriel.pdf
                    \includegraphics[0.9\textwidth]{MOTaffectationmateriel.pdf}

                \subsubsection{Processus Facturation}
                    %Ci-dessous le modèle conceptuel de traitement (MCT). --> FacturationMCT.pdf


                    %Ci-dessous le modèle organisationnel de traitement (MOT). --> FacturationMOT.pdf


                \subsubsection{Processus Maintenance}
                    %Ci-dessous le modèle conceptuel de traitement (MCT) --> MCT.pdf


                    %Ci-dessous le modèle organisationnel de traitement (MOT)  --> MOT.pdf
